    ...
    def __init__(self, wsi_object, top_left=None, bot_right=None, level=0,
        ...
        for cont_idx, contour in enumerate(wsi_object.contours_tissue):
            ...
            coord_results, _ = wsi_object.process_contour(contour, wsi_object.holes_tissue[cont_idx], level, '',
                                                          patch_size=patch_size[0], step_size=step_size[0],
                                                          contour_fn=cont_check_fn,
...
class WSIPatchDataset(Dataset):
    """Whole Slide Image Patch Dataset, which can obtain the features of each patch of WSIs."""

    def __init__(self, features_dir: str, label_csv: str, preload: bool = False):
        super(WSIPatchDataset, self).__init__()
        self.features_dir = features_dir
        self.label_csv_data = pd.read_csv(label_csv)
        self.slide_ids = [slide_id.split('.kfb')[0] for slide_id in self.label_csv_data['slide_id'].tolist()]
        self.labels = self.label_csv_data['label'].tolist()
        self.preload = preload

        if self.preload:
            self.patch_features = self._load_patch_features()

    def _load_patch_features(self) -> list:
        """Load all the patch features of all WSIs."""
        patch_features = []
        for slide_id in self.slide_ids:
            file_path = os.path.join(self.features_dir, f"{slide_id}.h5")
            try:
                with h5py.File(file_path, 'r') as f:
                    patch_feature = f['features'][()]
                    patch_features.append(torch.as_tensor(patch_feature, dtype=torch.float32))
            except Exception as e:
                print(f"Error loading features from {file_path}: {e}")
        return patch_features

    def __len__(self) -> int:
        return len(self.label_csv_data)
    ...
    def __getitem__(self, index: int):
        ...
        if self.preload:
            ...
            try:
                with h5py.File(file_path, 'r') as f:
                    patch_feature = f['features'][()]
                    return slide_id, torch.as_tensor(patch_feature, dtype=torch.float32), label
            except Exception as e:
                print(f"Error loading features from {file_path}: {e}")
                return None
